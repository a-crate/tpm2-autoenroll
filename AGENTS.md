...
The daemon can be run by hand with `--crypttab=`, `--ignore=`, and `--socket-dir=` overrides (defaults: `/etc/crypttab`, `/etc/tpm2-autoenroll/ignore`, `/run/cryptsetup-keys.d`).

## Architecture

The binary is `tpm2-autoenrolld`, a single-threaded daemon. It hooks into systemd-cryptsetup's key-discovery path: for each volume, systemd-cryptsetup connects to `/run/cryptsetup-keys.d/<volume>.key` if that path is a socket. The daemon binds one such socket per TPM2-bound crypttab entry (`crypttab.rs`, where an entry counts as TPM2-bound if it has `tpm2-device=`), minus the entries in the ignore list (`ignore.rs`). It sends `READY=1` (`notify.rs`) only once every socket is bound.
...
The module never lists volumes; the daemon reads them from crypttab at runtime. For each stage in `stages` (`initrd` and/or `systemd`) it installs:
...
`Wants=` is used rather than `Requires=` so that a daemon which fails to start costs the feature, not the boot. The `initrd` stage asserts that `boot.initrd.systemd.enable` and `boot.initrd.systemd.tpm2.enable` are set.
...
- Commit subjects are prefixed with a component: `daemon:`, `module:`, `test:`, `package:`, `crypttab:`, `docs:`.
